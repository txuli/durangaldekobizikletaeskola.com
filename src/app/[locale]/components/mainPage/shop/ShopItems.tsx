...
const ShopItems: FC<ShopProps> = ({ item = [], title, onAdd }) => {
    return (
        <div className='flex w-full relative mb-16'>
            <div className="bg-customblue font-fredoka text-white font-bold text-xl p-2  [writing-mode:vertical-rl] [text-orientation:upright] hidden sm:block absolute mt-4 h-[332] xl:flex items-center justify-center rounded-xl">
                <h3 className='uppercase'>{title}</h3>
            </div>
            <div className='px-3 grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 xl:grid-cols-6 gap-11 auto-rows-auto h-[332] w-full justify-center place-items-center xl:ml-24 2xl:ml-16'>
                {item.map(({ image, name, image2, add }, index) => (
                    <div className="w-50 h-[300px] group relative" key={index}>
                        <div className="w-full h-full bg-white rounded-t-xl flex items-center justify-center">
                            <div
                                className="absolute bottom-0 left-0 w-full hidden sm:flex items-center justify-center bg-black/40 bg-opacity-50 py-1 z-20 opacity-0 group-hover:opacity-100 transition-opacity duration-300"
                                onClick={() => onAdd && onAdd({ name, image })}
                            >
                                <span className="text-white text-2xl font-bold font-fredoka tracking-wider cursor-pointer uppercase">
                                    {add}
                                </span>
                            </div>
                        ...
                        <h4 className="font-fredoka uppercase text-center bg-custom-dark-blue text-2xl font-semibold rounded-b-xl">{name}</h4>
                    </div>
                ))}
            </div>
        </div>
