...
export default function News({ items }: NewsProps) {
  return (
    <Section className="flex flex-wrap gap-8 justify-center sm:grid sm:grid-cols-1 md:grid-cols-2 lg:grid-cols-2 xl:grid-cols-3  h-116">
      {items.map((item, index) => (
        <article
          key={index}  
          className="group relative h-96 w-86 hover:w-100 lg:hover:h-100 transition-all duration-300 ease-in-out  mx-auto "
        >
